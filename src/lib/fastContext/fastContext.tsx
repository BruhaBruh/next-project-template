...
export type UseStoreAction<State> = <
  Return = State,
...
export const createFastContext = <T extends Record<string | number | symbol, unknown>>(
  ssrState: T,
  name: string = '',
) => {
  ...
  const useStore = <State extends T, Return extends unknown = State>(
    selector?: (store: State) => Return,
  ): [Return, (value: Partial<State> | ((prev: State) => Partial<State>)) => void] => {
